  ...
  async getShow(tmdbId) {
    if (tmdb) {
      try {
        ...
        console.warn('⚠️ TMDb API failed, returning mock show detail:', e.message);
      }
    }
  ...
  async getSeason(tmdbId, seasonNumber) {
    if (tmdb) {
      try {
        ...
        console.warn('⚠️ TMDb API failed, returning mock season detail:', e.message);
      }
    }
  ...
  async getTrending() {
    if (tmdb) {
      try {
        ...
        console.warn('⚠️ TMDb API failed, returning mock trending:', e.message);
      }
    }
    return { results: MOCK_BACKEND_SHOWS };
  },
  ...
  async getPopular(page = 1) {
    if (tmdb) {
      try {
        ...
        console.warn('⚠️ TMDb API failed, returning mock popular:', e.message);
      }
    }
    return { results: MOCK_BACKEND_SHOWS };
  },
